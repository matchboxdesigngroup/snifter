 ...
 *
 * @link https://github.com/retlehs/sn/blob/master/doc/rewrites.md
 *
 ...
 */
function sn_add_rewrites( $content ) {
	global $wp_rewrite;
	$sn_new_non_wp_rules = array(
		'assets/(.*)'  => THEME_PATH . '/assets/$1',
		'plugins/(.*)' => RELATIVE_PLUGIN_PATH . '/$1',
	);
	$wp_rewrite->non_wp_rules = array_merge( $wp_rewrite->non_wp_rules, $sn_new_non_wp_rules );
	return $content;
} // sn_add_rewrites
 ...
 */
function sn_clean_urls( $content ) {
...
if ( ! is_multisite() && ! is_child_theme() ) {
	if ( current_theme_supports( 'rewrites' ) ) {
		add_action( 'generate_rewrite_rules', 'sn_add_rewrites' );
	} // if()
	...
	if ( ! is_admin() && current_theme_supports( 'rewrites' ) ) {
		...
		add_filters( $tags, 'sn_clean_urls' );
	} // if()
} // if()